...
def get_packages_section(repo_root: Path) -> str:
    ...
    if not pkg_info:
        ...
        # OpenSpec uses a different structure: openspec/specs/<spec-name>/spec.md
        # List available specs instead of "layers"
        openspec_dir = repo_root / "openspec" / "specs"
        if openspec_dir.is_dir():
            specs = sorted(d.name for d in openspec_dir.iterdir() if d.is_dir())
            if specs:
    ...
    for pkg in pkg_info:
        ...
        if pkg["specLayers"]:
            ...
            for layer in pkg["specLayers"]:
                lines.append(f"  - openspec/specs/{layer}/spec.md")
        else:
...
def get_context_packages_json(repo_root: Path | None = None) -> dict:
    ...
    if not pkg_info:
        ...
        layers = []
        if openspec_dir.is_dir():
            layers = sorted(d.name for d in openspec_dir.iterdir() if d.is_dir())
        return {
            "mode": "single-repo",
            "specLayers": layers,
        }
