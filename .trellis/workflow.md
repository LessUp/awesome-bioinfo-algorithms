...
This project uses **OpenSpec** for specifications, located at `openspec/specs/`.

- `openspec/specs/<spec-name>/spec.md` — Gherkin-style specifications with Given/When/Then scenarios
- Available specs: `algorithm-schema`, `cli-interface`, `testing-strategy`, `core-architecture`, `product-vision`

```bash
python3 ./.trellis/scripts/get_context.py --mode packages   # list available specs
```

**When to update spec**: new feature requirements · validation rules changes · CLI contract changes.
...
**What to put in**:
- **Spec files** — `openspec/specs/<spec-name>/spec.md` relevant to this task
...
Update the docs under `openspec/specs/` accordingly. Even if the conclusion is "nothing to update", walk through the judgment.
