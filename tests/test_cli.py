...
def test_main_returns_usage_when_no_command_is_provided(monkeypatch, capsys):
    """main() should print usage and fail when no command is given."""
    monkeypatch.setattr(cli.sys, 'argv', ['python'])

    exit_code = cli.main()
    output = capsys.readouterr().out

    assert exit_code == 1
    assert 'Usage: python -m scripts <command>' in output
    assert 'Commands:' in output



def test_main_rejects_unknown_command(monkeypatch, capsys):
    """main() should reject commands outside the dispatch table."""
    monkeypatch.setattr(cli.sys, 'argv', ['python', 'unknown'])
...
def test_main_dispatches_to_selected_command(monkeypatch):
    ...
    monkeypatch.setattr(cli, 'COMMANDS', {'validate': fake_validate})
    monkeypatch.setattr(cli.sys, 'argv', ['python', 'validate'])
