...
def generate_index(
    ...
    for i, algo in enumerate(latest):
        ...
        year_badge = f'<span class="aba-badge aba-badge-year">{algo["year"]}</span>' if algo.get("year") else ""
...
def generate_algo_page(algo: dict, cat_map: dict) -> str:
    ...
    parent_cat = sub.get("parent", {}).get("name", "") if sub.get("parent") else ""
...
'''
    ...
    tags_section = ""
    if algo.get("tags"):
        tag_links = " ".join(f'[<span class="aba-badge">{tag}</span>](tags.md#{tag})' for tag in algo["tags"])
        tags_section = f'''
### :material-tag: 标签
